...
#Calculate the k_shell value of nodes in the graph
def k_shell(graph):
...
def inflate(graph, subgraph_node):
    ...
    SG_neighbor_nodes = list(set(SG_neighbor_nodes).difference(set(subgraph_node)))  #Get neighbor nodes of the subgraph
    
    if len(subgraph_node) == 1:
        raw_fitness_SG = 0; raw_expectation_edge = 0
    else:
        inner_SG_weight, out_SG_weight, Modularity, edge_num = modularity(G,subgraph_node)     #Fitness score of the initial subgraph
    ...
    while SG_neighbor_nodes:
        ...
        for nei_node in SG_neighbor_nodes:
            ...
            for value2 in list_contact_nodes:
                edge_wei = G.get_edge_data(nei_node, value2)["weight"] #The edge weights between neighbor nodes and subgraph nodes
...
def modularity(graph, subgraph_node):
    ...
    if SG_neighbor_nodes:                               #Sum of external weights of the subgraph
...
#Calculate the density of the complex
...
def merge(complex_dict, fileName):
    ...
    for key1 in complex_dict:
        for k in range(key1+1,len(complex_dict)):
            ...
            if len(inter) == len(complex_dict[k][0]) and len(inter) == len(complex_dict[key1][0]):
                del_num.append(k)
...
def main():
    parser = argparse.ArgumentParser(description="identification protein complexes")
    parser.add_argument("--input_PPI_weight", action="store", dest="filename", required=True, 
                                    help="The filename of the PPI weight network")
    parser.add_argument("--outfile", action="store", dest="outfile", required=True, 
                                    help="FThe filename of the output complexes")
    ...
    #filename = "../Data/test_data/test_weight.txt"  ###PPI_weight file
    filename = args.filename
    outfile_first = "../test_temporary_end.txt"   ###Temporary cache file
    outfile = args.outfile    ###Complexes output file
    ...
    ###You can set the number of nodes for per thread
    threads = []
    for num in k_shell_dict.keys():
        k_shell_list = []
        k_shell_list.extend(k_shell_dict.get(num))      #Merge into one list of k_shell 
        if int(num) >= 10:                              #k-value of nodes
            step = 1                                    #Each thread contains 1 node that k_shell >= 20
            for i in range(0,len(k_shell_list),step):
                ...
                threads.append(t)
        elif int(num) >=5 and int(num) <10: 
            step = 2                                   #Each thread contains 2 nodes that k_shell >= 5 and < 20
            for i in range(0,len(k_shell_list),step):
                list_split_k_shell = []
                list_split_k_shell = k_shell_list[i:i+step]
                t = threading.Thread(target=calculate,args=(G, list_split_k_shell, outfile_first))
