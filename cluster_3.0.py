...
def inflate(graph, subgraph_node):
    ...
    SG_neighbor_nodes = list(set(SG_neighbor_nodes).difference(set(subgraph_node)))  #得到子图邻居节点
    
    if len(subgraph_node) == 1:
        raw_fitness_SG = 0; raw_expectation_edge = 0
    else:
        inner_SG_weight, out_SG_weight, Modularity, edge_num = modularity(G,subgraph_node)     #初始子图的fitness
        ...
        #raw_fitness_SG = fitness_SG = 1 / (1 / float(density) +1 / float(Modularity))
    ...
    while SG_neighbor_nodes:
        ...
        for nei_node in SG_neighbor_nodes:
            ...
            for value2 in list_contact_nodes:
                edge_wei = G.get_edge_data(nei_node, value2)["weight"] #得到邻居节点与子图节点的边权
...
def modularity(graph, subgraph_node):
    ...
    Modularity = float(inner_SG_weight) / (float(inner_SG_weight) + float(out_SG_weight))  #模块度值
    ...
    for value1 in k_shell_list:
        if(int(G.degree(value1)) >= int(1)):
            ...
            while fitness_SG > oldfitness_SG:
                ...
                subgraph_node, fitness_SG, expectation_edge = inflate(G, subgraph_node)  #subgraph_node是list
...
if __name__ == '__main__':
    ...
    filename = "/data/panyuliang/classification_methods/weight_graph/sum_weight.txt"
    G = constructGraph(filename)
    print("G is constracted done...")
    k_shell_dict = k_shell(G)
    print("k_shell_dict is done...")
    outfile_first = "/data/panyuliang/cluster/cluster_result/sum_temporary_end.txt" 

    print("threads stand by...")
    threads = []
    for num in k_shell_dict.keys():
        k_shell_list = []
        k_shell_list.extend(k_shell_dict.get(num))      #合并为一个k_shell 
        if int(num) >= 20:
            step = 1
    ...
    print("ALL end")
     ...
     ####生成文件去重
    outfile = "/data/panyuliang/cluster/cluster_result/sum_exlucde_duplicate_end.txt"
